...
const Arena = ({ characterNFT, setCharacterNFT }) => {
  ...
  useEffect(() => {
    ...
    const onAttackComplete = (newBossHp, newPlayerHp) => {
        ...
        if (playerHp<=0) console.log("RIP " + characterNFT.name);
  ...
  return (
    ...
    {boss && characterNFT && (
      <div id="toast" className={showToast ? 'show' : ''}>
        <div id="desc">{`💥 ${boss.name} was hit for ${characterNFT.spellDamage}!`}</div>
      </div>
    )}
    {boss && (
      ...
      <div className="players-container">
        <div className="player-container">
          ...
          <div className="player">
            ...
            <div className="stats">
              <h4>{`⚔️ Spell Damage: ${characterNFT.spellDamage}`}</h4>
            </div>
          </div>
        </div>
      </div>
